...
export const getCurrentAccount = <T>(): T | undefined => {
  ...
  if (userToken) {
    ...
    const expirationToken = (decodeToken as unknown as CommonUsuarioClaims)
      .expires_in;

    if (expirationToken < dateNow.getTime() / 1000) {
      const tokenDate = new Date(expirationToken).toLocaleDateString("pt-BR");
